<?php
	...
	if ($checkLogin) {
        if ($_GET["act"] == "view" && isset($_GET["message_id"]))
        {
            ...
            //echo('INN VIEW'.);
            $sql = "SELECT ".TABLE_MESSAGES.".*, ".TABLE_USERS.".user_name AS message_fromuser, ".TABLE_USERS.".user_groupid AS message_fromgroup FROM `".TABLE_MESSAGES."` LEFT JOIN `".TABLE_USERS."` ON ".TABLE_MESSAGES.".message_fromid = ".TABLE_USERS.".user_id WHERE message_toid = '0' AND message_id = '".$db->escape($_GET["message_id"])."' AND message_todelete = '0' ORDER BY message_time DESC, message_id DESC";
            $value = $db->query_first($sql);
    ?>
<div id="check_history">
<div class="section_title">PRIVATE MESSAGES</div>
<div class="section_title"><a href="?act=compose">Send P.M</a> | <a href="./messages.php">Inbox</a> | <a href="?act=sent">Sent</a></div>
...
<?php
    if ($value) {
        if ($value["message_status"] == 1) {
            ...
            $db->update(TABLE_MESSAGES, $message_update, "message_id='".$db->escape($_GET["message_id"])."'");
        }
    ?>
<tr>
...
<?php
    ...
    ?>
<span><a href="./users.php?txtUserid=<?=$value['message_fromid']?>&txtUsername=&txtUsermail=&txtUseryahoo=&lstUserbalance=&lstUsertype=&btnSearch=Search">(Manager)</a></span>
<?php
    }
    ?>
...
<?php
	} else if ($_GET["act"] == "compose") {
        //echo('INN COMPOSE');
    ?>
<div id="check_history">
<div class="section_title">PRIVATE MESSAGES</div>
<div class="section_title">Send P.M | <a href="./messages.php">Inbox</a> | <a href="?act=sent">Sent</a></div>
<div class="section_content">
<table class="content_table">
<tbody>
...
-------------------------------------------
<?=htmlentities($value['message_message'])?></textarea>
<?php
    } else {
    ?>
...
<?php
		...
		$db->update(TABLE_MESSAGES, $message_update, "message_id='".$message_id."' AND message_toid='0' AND message_todelete = '0'");
		if ($db->affected_rows > 0) {
    ?>
<script type="text/javascript">setTimeout("window.location = './messages.php'", 1000);</script>
<div id="check_history">
...
<div id="check_history">
<div class="section_title">PRIVATE MESSAGES</div>
<div class="section_title"><a href="?act=compose">Send P.M</a> | <a href="./messages.php">Inbox</a> | Sent</div>
...
<?php
        ...
        //echo('INN INIT');
		$sql = "SELECT count(*) FROM `".TABLE_MESSAGES."` WHERE message_toid = '0' AND message_todelete = '0' ORDER BY message_time DESC, message_id DESC";
		$totalRecords = $db->query_first($sql);
		$totalRecords = $totalRecords["count(*)"];
		$perPage = 25;
		$totalPage = ceil($totalRecords/$perPage);
		...
		$sql = "SELECT ".TABLE_MESSAGES.".*, ".TABLE_USERS.".user_name AS message_fromuser, ".TABLE_USERS.".user_groupid AS message_fromgroup FROM `".TABLE_MESSAGES."` LEFT JOIN `".TABLE_USERS."` ON ".TABLE_MESSAGES.".message_fromid = ".TABLE_USERS.".user_id WHERE message_toid = '0' AND message_todelete = '0' ORDER BY message_time DESC, message_id DESC LIMIT ".(($page-1)*$perPage).",".$perPage;
...
<div id="check_history">
<div class="section_title">PRIVATE MESSAGES</div>
<div class="section_title"><a href="?act=compose">Send P.M</a> | Inbox | <a href="?act=sent">Sent</a></div>
<div class="section_page_bar">
<?php
    if ($totalRecords > 0) {
        echo "Page:";
