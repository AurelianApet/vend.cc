...
<?php
			...
			if (isset($_POST["otheraccount_import_save"]) || isset($_POST["otheraccount_import_preview"])) {
				...
				/*
				else if ($_POST["otheraccount_sellerid"] == "" || $_POST["otheraccount_sellerid"] <= 1) {
					$errorMsg = "Please chose a valid seller";
				}
				else if ($_POST["otheraccount_categoryid"] == "" || $_POST["otheraccount_categoryid"] < 0) {
					$errorMsg = "Please input a valid account category";
				}
				*/
...
<?php
					...
					$otheraccount_import["otheraccount_sellerid"] = $lstSellerId;
					if ($lstCategoryId > 0) {
						$otheraccount_import["otheraccount_categoryid"] = $lstCategoryId;
					} else {
						$otheraccount_import["otheraccount_categoryid"] = "0";
					}
...
?>
<?php
					$sql = "SELECT * FROM `".TABLE_OTHER_CATEGORYS."` WHERE other_category_sellerid = '".$seller["user_id"]."'";
					$allCategory = $db->fetch_array($sql);
					if (count($allCategory) > 0) {
						foreach ($allCategory as $category) {
?>
												<option value="<?=$seller["user_id"]?>;<?=$category["other_category_id"]?>" <?=(strval($_POST["lstSellerCategory"]) == $seller["user_id"].";".$category["other_category_id"])?" selected":""?>><?=$seller['user_name']?> - <?=$category['other_category_name']?></option>
<?php
						}
					}
?>
...
?>	
									...
									<!--tr>
										<td colspan="3" class="error">
											CHECK ACCOUNT LIVE BEFORE IMPORT: <input type="checkbox" name="checklive" />
										</td>
									</tr-->
									<tr>
										<td colspan="3" class="centered">
											<input type="submit" name="otheraccount_import_preview" value="Preview" /><input type="submit" name="otheraccount_import_save" value="Import" /><input onclick="window.location='./otheraccountmarkets.php'"type="button" name="otheraccount_import_cancel" value="Cancel" />
										</td>
									</tr>
								</form>
							</tbody>
...
?>
				<div id="cards">
					<div class="section_title">EXPORT SELLER ACCOUNTS</div>
					<div class="section_content">
						<table class="content_table">
							<tbody>
								<tr>
									...
									<td class="centered">
										<p>
											<form method="POST" action="../otheraccountprocess.php">
												<label>
													<input name="export_seller_unsold" type="submit" class="bold" value="UnSold Accounts">
												</label>
												<span> | </span>
												<label>
													<input name="export_seller_sold" type="submit" class="bold" value="Sold Accounts">
												</label>
											</form>
										</p>
									</td>
...
<?php
		if (isset($_POST["otheraccount_edit_save"])) {
			if ($_POST["lstSellerCategory"] != "" && count(explode(";", $_POST["lstSellerCategory"])) == 2) {
				...
				$lstSellerId = $db->escape($lstSellerCategory[0]);
				$lstCategoryId = $db->escape($lstSellerCategory[1]);
				$otheraccount_update["otheraccount_sellerid"] = $lstSellerId;
				if ($lstCategoryId > 0) {
					$otheraccount_update["otheraccount_categoryid"] = $lstCategoryId;
				} else {
					$otheraccount_update["otheraccount_categoryid"] = "0";
				}
			} else {
				$errorMsg = "Please chose a valid Seller - Category";
			}
...
<?php
		...
		if (count($records)>0) {
			$value = $records[0];
			$value["lstSellerCategory"] = $value["otheraccount_sellerid"].";".$value["otheraccount_categoryid"];
?>
								<form method="POST" action="">
									<tr>
										<td colspan="4" class="centered">
...
<?php
				...
				if (count($allCategory) > 0) {
					foreach ($allCategory as $category) {
?>
												<option value="<?=$seller["user_id"]?>;<?=$category["other_category_id"]?>" <?=(strval($value["lstSellerCategory"]) == $seller["user_id"].";".$category["other_category_id"])?" selected":""?>><?=$seller['user_name']?> - <?=$category['other_category_name']?></option>
<?php
					}
				}
?>
...
?>
									...
									<tr>
										<td colspan="4" class="centered">
											<input type="submit" name="otheraccount_edit_save" value="Save" /><input onclick="window.location='./otheraccountmarkets.php'"type="button" name="otheraccount_edit_cancel" value="Cancel" />
										</td>
									</tr>
								</form>
<?php
			...
			if (!$errorMsg) {
				...
				if (count($otheraccount_update) > 0) {
					if($db->update(TABLE_OTHERACCOUNTS, $otheraccount_update, "otheraccount_id IN (".$otheraccounts_sql.") AND otheraccount_sellerid > '0'")) {
						$errorMsg = "Update Accounts error.";
					}
				}
			}
			if (!$errorMsg) {
...
?>
														...
														<td class="centered">
															<span class="bold" style="color:<?=$user_groups[$allSellers[$value['otheraccount_sellerid']]["user_groupid"]]["group_color"]?>"><?=$allSellers[$value['otheraccount_sellerid']]["user_name"]?></span>
														</td>
														<td class="centered">
															<span>
<?php
...
?>
				<script type="text/javascript">setTimeout("window.location = './otheraccountmarkets.php'", 1000);</script>
				<div id="cards">
...
END;
		...
		if ($_GET["lstSellerCategory"] != "" && count(explode(";", $_GET["lstSellerCategory"])) == 2) {
			...
			$lstCategoryId = $db->escape($lstSellerCategory[1]);
			if ($lstCategoryId > 0) {
				$searchSellerCategory = "otheraccount_sellerid = '".$lstSellerId."' AND otheraccount_categoryid = '".$lstCategoryId."'";
			} else if (strval($lstCategoryId) == "0") {
				$searchSellerCategory = "otheraccount_sellerid = '".$lstSellerId."' AND otheraccount_categoryid NOT IN (SELECT other_category_id FROM `".TABLE_OTHER_CATEGORYS."` WHERE other_category_sellerid = '".$lstSellerId."')";
			} else {
				$searchSellerCategory = "otheraccount_sellerid = '".$lstSellerId."'";
			}
		} else {
			$_GET["lstSellerCategory"] = "";
			$searchSellerCategory = "otheraccount_sellerid > 0";
		}
		$currentGet = "lstSellerCategory=".$_GET["lstSellerCategory"]."&";
		$currentGet .= "txtInfo=".$_GET["txtInfo"]."&";
		$_GET["lstType"] = trim($_GET["lstType"]);
		if (isset($_GET["btnSearch"])) {
			$currentGet .= "lstType=".$_GET["lstType"];
			$currentGet .= "&lstAvailable=".$_GET["lstAvailable"]."&lstStatus=".$_GET["lstStatus"]."&lstCheck=".$_GET["lstCheck"];
		...
		$group_where = $searchSellerCategory." AND ".$searchType." AND ".$searchStatus." AND ".$searchCheck." AND ".$searchAvailable;
				...
				<div id="search_cards">
					...
					<div class="section_content">
						<table class="content_table centered">
							<tbody>
								<form name="search" method="GET" action="otheraccountmarkets.php">
									<tr>
									...
									<tr>
										...
										<td class="formstyle">
											<span class="bold">SELLER - CATEGORY</span>
										</td>
										<td class="formstyle">
...
<?php
				...
				if (count($allCategory) > 0) {
					foreach ($allCategory as $category) {
?>
												<option value="<?=$seller["user_id"]?>;<?=$category["other_category_id"]?>" <?=(strval($_GET["lstSellerCategory"]) == $seller["user_id"].";".$category["other_category_id"])?" selected":""?>><?=$seller['user_name']?> - <?=$category['other_category_name']?></option>
<?php
					}
				}
?>
