...
<?php
		if (isset($_POST["config_edit_save"])) {
			...
			if ($errorMsg == "") {
				if($db->update(TABLE_CONFIGS, $config_update, "config_id='".$config_id."'")) {
					$errorMsg = "";
				}
				else {
...
?>
									...
									<tr>
										<td colspan="3" class="centered">
											<span class="success">Update Config successfully.</span>
										</td>
									</tr>
<?php
			}
...
?>
								<tr>
									<td class="configs_title">
										<span class="error">Config ID Invalid.</span>
									</td>
								</tr>
<?php
		}
...
<?php
		...
		$list_configs = $db->fetch_array($sql);
?>
				<div id="config_manager">
...
<?php
		if (count($list_configs) > 0) {
			foreach ($list_configs as $key=>$value) {
?>
								<tr class="formstyle">
									<td class="centered">
										<span><?=$value['config_id']?></span>
