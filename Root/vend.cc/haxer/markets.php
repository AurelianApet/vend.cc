...
<?php
			if (isset($_POST["card_import_save"]) || isset($_POST["card_import_preview"])) {
				...
				else if ($_POST["lstSellerCategory"] == "" || count(explode(";", $_POST["lstSellerCategory"])) != 2) {
					$errorMsg = "Please chose a valid Seller - Category";
				}
				/*
				else if ($_POST["card_sellerid"] == "" || $_POST["card_sellerid"] <= 1) {
					$errorMsg = "Please chose a valid seller";
				}
...
<?php
					...
					$lstSellerCategory = explode(";", $_POST["lstSellerCategory"]);
					$lstSellerId = $db->escape($lstSellerCategory[0]);
					$lstCategoryId = $db->escape($lstSellerCategory[1]);
					$card_import["card_sellerid"] = $lstSellerId;
					if ($lstCategoryId > 0) {
						$card_import["card_categoryid"] = $lstCategoryId;
					} else {
						$card_import["card_categoryid"] = "0";
					}
...
HTML;
							...
							if (isset($_POST["card_import_save"])) {
								$sql = "SELECT count(*) FROM `".TABLE_CARDS."` WHERE card_number = AES_ENCRYPT('".$db->escape($card_import["card_number"])."', '".strval(DB_ENCRYPT_PASS)."') AND card_sellerid > '0'";
								...
								<form method="POST" action="">
									...
									<tr>
										<td class="centered bold">
											<select name="lstSellerCategory" class="formstyle" id="lstSellerCategory">
<?php
		if (count($allSellers) > 0) {
			foreach ($allSellers as $seller) {
?>
<?php
				$sql = "SELECT * FROM `".TABLE_CATEGORYS."` WHERE category_sellerid = '".$seller["user_id"]."'";
				$allCategory = $db->fetch_array($sql);
				if (count($allCategory) > 0) {
					foreach ($allCategory as $category) {
?>
												<option value="<?=$seller["user_id"]?>;<?=$category["category_id"]?>" <?=(strval($_POST["lstSellerCategory"]) == $seller["user_id"].";".$category["category_id"])?" selected":""?>><?=$seller['user_name']?> - <?=$category['category_name']?></option>
<?php
					}
				}
?>
												<option value="<?=$seller["user_id"]?>;0" <?=(strval($_POST["lstSellerCategory"]) == $seller["user_id"].";0")?" selected":""?>><?=$seller['user_name']?> - (No Category)</option>
<?php
			}
		}
?>
									...
									<tr>
										<td colspan="8" class="centered">
											<input type="submit" name="card_import_preview" value="Preview" /><input type="submit" name="card_import_save" value="Import" /><input onclick="window.location='./markets.php'"type="button" name="card_import_cancel" value="Cancel" />
										</td>
									</tr>
								</form>
							</tbody>
...
?>
				<div id="cards">
					<div class="section_title">EXPORT SELLER CARDS</div>
					<div class="section_content">
						<table class="content_table">
							<tbody>
								<tr>
									...
									<td class="centered">
										<p>
											<form method="POST" action="../cardprocess.php">
												<label>
													<input name="export_seller_unsold" type="submit" class="bold" value="Seller UnSold Cards">
												</label>
												<span> | </span>
												<label>
													<input name="export_seller_sold" type="submit" class="bold" value="Seller Sold Cards">
												</label>
												<span> | </span>
												<label>
													<input name="export_seller_expired" type="submit" class="bold" value="Seller Expired Cards">
												</label>
											</form>
										</p>
									</td>
...
<?php
		if (isset($_POST["card_edit_save"])) {
			if ($_POST["lstSellerCategory"] != "" && count(explode(";", $_POST["lstSellerCategory"])) == 2) {
				$lstSellerCategory = explode(";", $_POST["lstSellerCategory"]);
				$lstSellerId = $db->escape($lstSellerCategory[0]);
				$lstCategoryId = $db->escape($lstSellerCategory[1]);
				$card_update["card_sellerid"] = $lstSellerId;
				if ($lstCategoryId > 0) {
					$card_update["card_categoryid"] = $lstCategoryId;
				} else {
					$card_update["card_categoryid"] = "0";
				}
			} else {
				$errorMsg = "Please chose a valid Seller - Category";
			}
...
<?php
		...
		if (count($records)>0) {
			$value = $records[0];
			$value["lstSellerCategory"] = $value["card_sellerid"].";".$value["card_categoryid"];
?>
								<form method="POST" action="">
									<tr>
										<td colspan="4" class="centered">
...
<?php
				...
				if (count($allCategory) > 0) {
					foreach ($allCategory as $category) {
?>
												<option value="<?=$seller["user_id"]?>;<?=$category["category_id"]?>" <?=(strval($value["lstSellerCategory"]) == $seller["user_id"].";".$category["category_id"])?" selected":""?>><?=$seller['user_name']?> - <?=$category['category_name']?></option>
<?php
					}
				}
?>
												<option value="<?=$seller["user_id"]?>;0" <?=(strval($value["lstSellerCategory"]) == $seller["user_id"].";0")?" selected":""?>><?=$seller['user_name']?> - (No Category)</option>
<?php
			}
		}
?>
...
?>
									...
									<tr>
										<td colspan="4" class="centered">
											<input type="submit" name="card_edit_save" value="Save" /><input onclick="window.location='./markets.php'"type="button" name="card_edit_cancel" value="Cancel" />
										</td>
									</tr>
								</form>
<?php
...
?>
														...
														<td class="centered">
															<span class="bold" style="color:<?=$user_groups[$allSellers[$value['card_sellerid']]["user_groupid"]]["group_color"]?>"><?=$allSellers[$value['card_sellerid']]["user_name"]?></span>
														</td>
														<td class="centered">
															<span>
<?php
...
?>
				<script type="text/javascript">setTimeout("window.location = './markets.php'", 1000);</script>
				<div id="cards">
...
END;
		...
		if ($_GET["lstSellerCategory"] != "" && count(explode(";", $_GET["lstSellerCategory"])) == 2) {
			$lstSellerCategory = explode(";", $_GET["lstSellerCategory"]);
			$lstSellerId = $db->escape($lstSellerCategory[0]);
			$lstCategoryId = $db->escape($lstSellerCategory[1]);
			if ($lstCategoryId > 0) {
				$searchSellerCategory = "card_sellerid = '".$lstSellerId."' AND card_categoryid = '".$lstCategoryId."'";
			} else if (strval($lstCategoryId) == "0") {
				$searchSellerCategory = "card_sellerid = '".$lstSellerId."' AND card_categoryid NOT IN (SELECT category_id FROM `".TABLE_CATEGORYS."` WHERE category_sellerid = '".$lstSellerId."')";
			} else {
				$searchSellerCategory = "card_sellerid = '".$lstSellerId."'";
			}
		} else {
			$_GET["lstSellerCategory"] = "";
			$searchSellerCategory = "1";
		}
		if (isset($_GET["btnSearch"])) {
			$currentGet = "";
			$currentGet .= "txtBin=".$_GET["txtBin"]."&lstSellerCategory=".$_GET["lstSellerCategory"]."&lstCountry=".$_GET["lstCountry"]."&lstState=".$_GET["lstState"]."&lstCity=".$_GET["lstCity"]."&txtZip=".$_GET["txtZip"]."&lstAvailable=".$_GET["lstAvailable"]."&lstExpire=".$_GET["lstExpire"]."&lstStatus=".$_GET["lstStatus"]."&lstCheck=".$_GET["lstCheck"];
		...
		$sql = "SELECT count(*) FROM `".TABLE_CARDS."` WHERE ".$searchSellerCategory." AND ".$searchExpire." AND ".$searchStatus." AND ".$searchCheck." AND ".$searchAvailable." AND ('".$searchBin."'='' OR AES_DECRYPT(card_number, '".strval(DB_ENCRYPT_PASS)."') LIKE '".$searchBin."%') AND ('".$searchCountry."'='' OR card_country = '".$searchCountry."') AND ('".$searchState."'='' OR card_state = '".$searchState."') AND ('".$searchCity."'='' OR card_city = '".$searchCity."') AND ('".$searchZip."'='' OR card_zip LIKE '".$searchZip."%')".$searchSSN.$searchDob." AND card_sellerid > '0'";
		...
		$sql = "SELECT ".TABLE_CARDS.".*, AES_DECRYPT(".TABLE_CARDS.".card_number, '".strval(DB_ENCRYPT_PASS)."') AS card_number, ".TABLE_USERS.".user_name, ".TABLE_GROUPS.".group_color, ".TABLE_CATEGORYS.".* FROM `".TABLE_CARDS."` LEFT JOIN `".TABLE_USERS."` ON ".TABLE_CARDS.".card_userid = ".TABLE_USERS.".user_id LEFT JOIN `".TABLE_GROUPS."` ON ".TABLE_USERS.".user_groupid = ".TABLE_GROUPS.".group_id LEFT JOIN `".TABLE_CATEGORYS."` ON ".TABLE_CARDS.".card_categoryid = ".TABLE_CATEGORYS.".category_id WHERE ".$searchSellerCategory." AND ".$searchExpire." AND ".$searchStatus." AND ".$searchCheck." AND ".$searchAvailable." AND ('".$searchBin."'='' OR AES_DECRYPT(card_number, '".strval(DB_ENCRYPT_PASS)."') LIKE '".$searchBin."%') AND ('".$searchCountry."'='' OR card_country = '".$searchCountry."') AND ('".$searchState."'='' OR card_state = '".$searchState."') AND ('".$searchCity."'='' OR card_city = '".$searchCity."') AND ('".$searchZip."'='' OR card_zip LIKE '".$searchZip."%')".$searchSSN.$searchDob." AND card_sellerid > '0' ORDER BY card_id LIMIT ".(($page-1)*$perPage).",".$perPage;
				...
				<div id="search_cards">
					...
					<div class="section_content">
						<table class="content_table centered">
							<tbody>
								<form name="search" method="GET" action="markets.php">
									<tr>
										<td class="formstyle">
											<span class="bold">CARD NUMBER</span>
										</td>
										<td class="formstyle">
											<span class="bold">SELER - CATEGORY</span>
										</td>
										<td class="formstyle">
									...
									<tr>
										...
										<td>
											<select name="lstSellerCategory" class="formstyle" id="lstSellerCategory">
												<option value="">(All Seller) - (All Category)</option>
<?php
		if (count($allSellers) > 0) {
			foreach ($allSellers as $seller) {
?>
												<option value="<?=$seller["user_id"]?>;" <?=(strval($_GET["lstSellerCategory"]) == $seller["user_id"].";")?" selected":""?>><?=$seller['user_name']?> - (All Category)</option>
<?php
				$sql = "SELECT * FROM `".TABLE_CATEGORYS."` WHERE category_sellerid = '".$seller["user_id"]."'";
				$allCategory = $db->fetch_array($sql);
				if (count($allCategory) > 0) {
					foreach ($allCategory as $category) {
?>
												<option value="<?=$seller["user_id"]?>;<?=$category["category_id"]?>" <?=(strval($_GET["lstSellerCategory"]) == $seller["user_id"].";".$category["category_id"])?" selected":""?>><?=$seller['user_name']?> - <?=$category['category_name']?></option>
<?php
					}
				}
?>
												<option value="<?=$seller["user_id"]?>;0" <?=(strval($_GET["lstSellerCategory"]) == $seller["user_id"].";0")?" selected":""?>><?=$seller['user_name']?> - (No Category)</option>
<?php
			}
		}
?>
...
<?php
		$sql = "SELECT DISTINCT card_country FROM `".TABLE_CARDS."` WHERE card_sellerid > '0'";
...
<?php
		$sql = "SELECT DISTINCT card_state FROM `".TABLE_CARDS."` WHERE card_sellerid > '0'";
		$allCountry = $db->fetch_array($sql);
		if (count($allCountry) > 0) {
			foreach ($allCountry as $country) {
...
<?php
		$sql = "SELECT DISTINCT card_city FROM `".TABLE_CARDS."` WHERE card_sellerid > '0'";
		$allCountry = $db->fetch_array($sql);
		if (count($allCountry) > 0) {
			foreach ($allCountry as $country) {
