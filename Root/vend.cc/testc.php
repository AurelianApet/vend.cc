...
/* Search Logic 
//category_id=&stagnant=&txtBin=403247&lstCity=&lstState=&txtZip=&lstCountry=&lstSeller=&btnSearch=Search

if ($_GET["category_id"] > 0) {
	$searchCatID = "card_categoryid = '".$_GET["category_id"]."'";}
	
if ($_GET["category_id"] > 0) {
	$searchCatID = "card_categoryid = '".$_GET["category_id"]."'";}
*/

/*WHERE card_sellerid >= 0 AND card_status = '0' AND card_userid = '0' AND ((cards.card_bin LIKE 403247) AND (''='' OR cards.card_country = '') AND (''='' OR cards.card_state = '') AND (''='' OR cards.card_city = '') AND (''='' OR cards.card_zip LIKE '%') LIMIT 0,20*/
...
//echo('<br /> $category ='.$searchCategory);	
...
//echo('<br /> $Binwhere ='.$searchBinWhere);		
		...
		foreach ($searchBins as &$searchBin) {
			if ($count_bin > $max_search_bin) {
				...
				if (strlen($searchBin) > 0) {
					...
					$searchBinWhere .= "".TABLE_CARDS.".card_bin LIKE ".$_GET["txtBin"];
				}
				$count_bin++;
			}
		}
	}
//echo('<br /> $Binwhere ='.$searchBinWhere.'<br>');		

	$search_where = $searchBinWhere.")";
//echo('<br /> $search_where ='.$search_where);
...
//echo('<br /> $group_where ='.$group_where);
$search_where .=$searchSSN.$searchDob;
	
//echo('<br /> $search_where5 ='.$search_where);	
...
//echo('<br /> $search_where6 ='.$search_where);
	
	
	//$sql = "SELECT count(*) FROM `".TABLE_CARDS."` WHERE 1=1 AND ".$group_where." AND ";
...
$searchwhere2="Where 1=1 ";
if(!empty($_GET["txtBin"])) {
$searchwhere2 .=" AND ".TABLE_CARDS.".card_bin LIKE ".$_GET["txtBin"];
}
if(!empty($_GET["lstCity"])) {
		$searchwhere2 .= " AND ".TABLE_CARDS.".card_city LIKE ".$_GET["lstCity"];
}

//echo("Boxx = ".$_GET["boxSSN"]);
$searchwhere2 .= ($_GET["boxSSN"] == "on")?" AND card_ssn <> ''":"";
$searchwhere2 .= ($_GET["boxDob"] == "on")?" AND card_dob <> ''":"";
...
?>
				<div id="search_cards">
					...
					<div class="section_content">
						<div>
								<form id="searchForm" name="searchForm" method="GET" action="testc.php">
